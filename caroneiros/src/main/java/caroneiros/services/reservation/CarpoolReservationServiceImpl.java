...
@Service
@Log4j2
public class CarpoolReservationServiceImpl implements CarpoolReservationService {
    ...
    public ReservationResponseDTO reserveCarpool(Long passengerId, Long carpoolId, ReservationRequestDTO dto) {
        ...
        if (carpool.getDriver().getId().equals(passengerId))
            throw new InvalidOperationException("O motorista não pode ser reservar sua própia carona!");
